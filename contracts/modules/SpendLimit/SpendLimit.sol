...
contract SpendLimit {
    ...
    struct Limit {
     ...
     mapping(address => Limit) public limits; // account => token => Limit
    ...
    function _updateLimit(address _token, uint _limit, uint _available, uint _resetTime, bool _isEnabled) public {
    ...
    function _checkSpendingLimit(address _token, uint _amount, bytes memory _data) internal returns(bool) {

        // ETH Transfer
        if ( _amount > 0 ) {
          _checkSpendingLimit(address(ETH_TOKEN_SYSTEM_CONTRACT), _amount);
        } 

        // ERC20 Transfer
        if ( BytesLib.getSelector(_data) == ERC20_TRANSFER_SELECTOR ) {
          (, _amount) = BytesLib.decodeERC20TransferArgs(_data);
          _checkSpendingLimit(_token, _amount);
